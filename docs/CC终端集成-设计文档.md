...
> 版本：v1.1 定稿（2026-08-15 评审修订：5 项需求澄清落回，新增 D10–D14，见 §10；分支 `v1.1-cc-console`）
...
| D10 | 快捷键让渡 | 终端 Tab 激活时，全部键盘输入（含 Ctrl+S / Ctrl+W / Ctrl+N 等全局快捷键）直达 pty；MeWork 全局快捷键不拦截（关闭靠 ✕ / 右键） |
| D11 | 退出/切换静默关闭 | 退出应用 / 切换工作区时，运行中终端会话**不提示、直接 kill**（与关闭终端 Tab 无二次确认一致） |
| D12 | xterm 生命周期 | 终端 Tab 切走时 xterm **常驻不卸载**（输出直写、滚动位置不丢），不做环形缓冲重放 |
| D13 | 自动重载不记版 | watch 触发的外部改动自动重载磁盘内容**不产生版本快照**（版本仅在用户主动保存时记录） |
| D14 | 并发上限 | **不设上限**（维持原定案；单人本地自用，不做资源限制） |
...
- **生命周期（D12）**：Tab 切走时 xterm 实例**常驻不卸载**（终端持续运行，输出直接写入，切回保留滚动位置）；Tab 关闭时才 `term:kill`
- 终端配色：**固定深色**（2026-08-15 用户定案——claude/Shell 的 ANSI 配色按深色背景设计，跟随 MeWork 亮色主题会「相反」；背景 `#1e1e1e`、前景 `#d4d4d4`、光标 accent 蓝，不随主题切换）；字号沿用编辑器字号（§10）
...
- **快捷键让渡（D10）**：`activeTab.type === 'terminal'` 时，键盘事件全部直达 pty；MeWork 全局快捷键（Ctrl+S 保存 / Ctrl+W 关标签 / Ctrl+N 新建等）不拦截
...
| 应用退出 | `before-quit` 统一 kill 全部 pty（**不提示**，D11） |
| 工作区切换 | 复用 `close()`：关闭全部终端 Tab（进程一并结束，**不提示**，D11） |
| 文件夹删除 | cwd 受影响的终端 Tab 一并关闭（进程 cwd 无法迁移） |
| 文件夹重命名 | **不处理**（2026-08-15 定案）：终端进程持有的是绝对路径 cwd，重命名不影响运行中进程；Tab 记录不做跟随、不加额外逻辑 |
...
**跳转后的内容处理**（2026-08-15 定案）：点击 badge 跳到文件 Tab 时——文件未脏（content === savedContent）→ 自动从磁盘重载；已脏 → 显示现有 externalChange 提示，由用户选择覆盖或放弃。

**自动重载与版本历史（D13）**：watch 触发的外部改动自动重载磁盘内容，**不产生版本快照**（`.wr/versions/` 不变）；版本快照仅在用户主动保存时产生。
...
> **架构决策**：新增 `term:` 前缀超出 PRD §3.1.2 / §7.4 既定预留（`agent:` / `llm:`）；`fs:watch_sync` / `fs:watch_changed` 亦为新增通道（现有 `fs:` 前缀下扩展）——按 CLAUDE.md 约定，落地时一并升 PRD 版本登记。
...
| 终端并发不设上限（D14）的资源占用 | 单人本地自用，相信不会超开；若未来出现内存压力，再评估软上限（§8 预留） |
| 旧持久化格式（relPath[]）| 读取兼容：string 元素视为 file tab（§3.1） |

> 文件夹重命名对终端的影响：已定案不处理（进程 cwd 为绝对路径，不受影响），风险表中不再列入。
...
## 10. 已定案事项（2026-08-15 用户确认）

- [x] badge 跳转后的重载策略：未脏自动重载、已脏走 existing externalChange 提示（§3.7）
- [x] 终端视觉：不入设置页；xterm **固定深色**（2026-08-15 实测定案：跟随主题与 claude 深色 ANSI 配色相反，改为固定深色），字号沿用编辑器字号
- [x] 终端并发数：**不设上限**
- [x] 文件夹重命名：**保留终端进程、不做任何处理**（无额外逻辑，§3.5）
- [x] 评审澄清（2026-08-15）：快捷键全让渡（D10）、退出/切换静默关闭（D11）、xterm 常驻不卸载（D12）、自动重载不记版（D13）、并发不设上限维持（D14）
