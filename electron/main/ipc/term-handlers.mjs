...
export function registerTermHandlers() {
  ...
  ipcMain.handle(IPC.term.checkCli, async () => {
    return cli.detect()
  })

  // 创建终端：cwdRelPath 为工作区相对路径（'.' 即根），沙箱解析后 spawn claude CLI
  ipcMain.handle(IPC.term.create, async (e, cwdRelPath) => {
    try {
      ...
      const termId = manager.create({
        ...
        args: cliInfo.args,
        cwd,
        onData: (id, chunk) => e.sender.send(IPC.term.dataEvent, id, chunk),
        onExit: (id, code) => e.sender.send(IPC.term.exitEvent, id, code)
      })
  ...
  ipcMain.handle(IPC.term.write, (_e, termId, data) => {
    manager.write(termId, data)
    return { ok: true }
  })

  // 终端尺寸同步（渲染层 xterm fit 后回传 cols/rows）
  ipcMain.handle(IPC.term.resize, (_e, termId, cols, rows) => {
    manager.resize(termId, cols, rows)
    return { ok: true }
  })

  // 关闭终端：杀 pty 进程
  ipcMain.handle(IPC.term.kill, (_e, termId) => {
    manager.kill(termId)
    return { ok: true }
  })
}
